/* Villar JA — mock data (Dominican e-CF context) */
...
const COMARK = ['#a60005','#1f9d57','#2563c9','#6b3fa0','#d9700a','#0e8a8a','#b8860b','#c2185b'];
...
function markFor(c){ return COMARK[c.mark % COMARK.length]; }
window.markFor = markFor;
window.COMARK = COMARK;

// e-CF emitted records
const ESTADOS = ['accepted','sent','rejected','pending','draft','contingency'];
const ESTADO_MAP = {
...
function buildFacturas(n){
  ...
  for (let i=0;i<n;i++){
    ...
    const cli = window.CLIENTES[Math.floor(rnd(i*5+2)*window.CLIENTES.length)];
    let estado;
    const er = rnd(i*7+3);
    if (er<0.62) estado='accepted'; else if (er<0.78) estado='sent'; else if (er<0.86) estado='pending';
    else if (er<0.92) estado='rejected'; else if (er<0.97) estado='contingency'; else estado='draft';
...
window.FACTURAS = buildFacturas(140);

function fmtDOP(n){ return new Intl.NumberFormat('es-DO',{minimumFractionDigits:2, maximumFractionDigits:2}).format(n); }
function fmtNum(n){ return new Intl.NumberFormat('es-DO').format(n); }
function fmtDate(d){ return new Intl.DateTimeFormat('es-DO',{day:'2-digit',month:'short',year:'numeric'}).format(d); }
function fmtDateTime(d){ return new Intl.DateTimeFormat('es-DO',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'}).format(d); }
window.fmtDOP = fmtDOP; window.fmtNum = fmtNum; window.fmtDate = fmtDate; window.fmtDateTime = fmtDateTime;

// 30-day line series (facturas emitidas)
window.SERIE_30D = (function(){
  const arr = [];
  for (let i=0;i<30;i++){
    const weekend = (i%7===5||i%7===6);
    const base = 3800 + Math.sin(i/3.2)*900 + i*42;
    const v = Math.round(base * (weekend?0.55:1) + rnd(i*9+4)*700);
    arr.push(v);
  }
  return arr;
})();

// Donut: distribución por tipo e-CF
window.DONUT_TIPOS = [
...
// DGII services status
window.DGII_SERVICES = [
...
// Contingencia retry queue
window.CONTINGENCIA_QUEUE = [
