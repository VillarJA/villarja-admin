  ...
  return (
    <div className="content-wrap fade-in">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--text-2)' }}>
              {activa
                ? 'DGII respondió 503 a las 12:33. Los e-CF se almacenan firmados y se reenvían automáticamente.'
                : 'Todos los servicios DGII operativos. Sin elementos en cola.'}
            </div>
          </div>
          <button
            className={`btn${activa ? ' primary' : ''}`}
            onClick={() => {
              setActiva((a) => !a);
              toast(activa ? 'Saliendo de contingencia…' : 'Contingencia activada manualmente');
            }}
          >
            <Icon name="refresh" />
            {activa ? 'Forzar reenvío' : 'Activar manual'}
          </button>
        </div>
      </div>
      ...
      <div className="grid" style={{ gridTemplateColumns: 'repeat(4,1fr)', marginBottom: 18 }}>
        ...
        <KPICard icon="refresh"    iconBg="var(--info-bg)" iconColor="var(--info)" label="Reintentos / hora"    value="124" />
        <KPICard icon="checkcircle" iconBg="var(--ok-bg)"  iconColor="var(--ok)"  label="Reenviados hoy"       value="296" />
        <KPICard icon="clock"      iconBg="var(--warn-bg)" iconColor="var(--warn)" label="Antigüedad máx."     value="11" unit="min" />
      </div>
        ...
        <div className="card">
          <div className="card-head">
            ...
            <span className="badge cont">
              <i className="bdot" />{queue.length} pendientes
            </span>
          </div>
