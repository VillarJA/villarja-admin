...
export async function GET(req: NextRequest, { params }: { params: Promise<{ caseId: string }> }) {
  ...
  try {
    const upstream = await fetch(`${ECF_BASE}/api/v1/certification/simulation/${caseId}/pdf`, {
    ...
    const cd = upstream.headers.get('content-disposition') ?? 'attachment; filename="ecf.pdf"';
